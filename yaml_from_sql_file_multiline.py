...
def yaml_from_sql_file():
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".sql"):
            ...
            with open(os.path.join(directory_path, filename), "r") as sql_file:
                ...
                cleaned_sql_query = re.sub(r'/\*[^*]*\*+(?:[^/*][^*]*\*+)*/', '', sql_query)

                # Remove empty lines
                cleaned_sql_query = '\n'.join([line for line in cleaned_sql_query.split('\n') if line.strip() != ''])
                print(f" {filename} -  {len(cleaned_sql_query.splitlines())}")

                formatted_sql = sqlparse.format(cleaned_sql_query, reindent=True, keyword_case='upper')
    ...
    target_filename = "C:\\Users\\Krishna\\PycharmProjects\\yamlsql\\output\\" + "combined_sql_files1.yaml"
